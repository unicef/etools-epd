...
import {RootState} from '../../pages/interventions/intervention-tab-pages/common/types/store.types';
...
export class LanguagesDropdown extends connect(store)(LitElement) {
  ...
  private updateUserPreference(language: string) {
    sendRequest({
      endpoint: etoolsEndpoints.userProfile as any,
      method: 'PATCH',
      body: {preferences: {language: language}}
    })
      .then((response) => {
