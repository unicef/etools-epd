...
export class CommentsDialog extends connect(getStore())(LitElement) {
  ...
  @property() commentsDialogTitle = 'Comments on: Overall Cache Budget';
  @property() dialogOpened = true;
  @property() comments: (InterventionComment & {loadingError?: boolean})[] = [];

  set dialogData({interventionId, relatedTo}: any) {
    this.interventionId = interventionId;
    this.relatedTo = relatedTo;
    const comments: InterventionComment[] = getStore().getState().commentsData.comments[relatedTo] || [];
  ...
  resolveComment(id: number, index: number): void {
    ...
    sendRequest({
      ...
      .then((updatedComment: InterventionComment) => {
        ...
        getStore().dispatch(updateComment(this.relatedTo, updatedComment));
        this.requestUpdate();
      })
      .catch(() => {
        this.resolvingCollection.delete(id);
  ...
  addComment(comment?: any): void {
    ...
    sendRequest({
      ...
      .then((newComment: InterventionComment) => {
        ...
        getStore().dispatch(addComment(this.relatedTo, newComment));
        this.requestUpdate().then(() => {
          if (comment) {
            this.scrollDown();
          }
